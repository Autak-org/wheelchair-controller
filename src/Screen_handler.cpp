...
void createScreen(uint16_t speed, bool mode, TFT_eSPI *tft, TFT_eSprite *img)
{
  ...
  img->createSprite(30, 50);
  img->fillSprite(0xf80c);
  img->pushSprite(280 + offset_x, 105, TFT_BLACK);
  img->deleteSprite();
  ...
  img->pushSprite(80 + offset_x, 180);
  img->deleteSprite();
  ...
  img->pushSprite(80 + offset_x, 10, 0xf8aa);
  img->deleteSprite();
};
...
void displayBatteries(float v1, float v2, TFT_eSPI *tft, TFT_eSprite *img)
{
  ...
  if (v1 / maximumVoltage > 0.5)
    color = TFT_GREEN;
  else if (v1 / maximumVoltage < 0.5 && v1 / maximumVoltage > 0.2)
    color = TFT_ORANGE;
  else
    color = TFT_RED;
  ...
  if (v2 / maximumVoltage > 0.5)
    color = TFT_GREEN;
  else if (v2 / maximumVoltage < 0.5 && v2 / maximumVoltage > 0.2)
    color = TFT_ORANGE;
  else
    color = TFT_RED;
...
void configureMode(TFT_eSPI *tft, TFT_eSprite *img)
{
  ...
  case 0:
    ...
    if (!calibrating)
    {
      ...
      img->pushSprite(80 + offset_x, 180, TFT_BLACK);
      img->deleteSprite();
      img->createSprite(200, 100);
      img->fillSprite(0xf80c);
      img->setCursor(10, 10);
    ...
    else
    {
      ...
      if ((currentTime - startingTime) < 8000)
      {
        ...
        if (currentTime - startingTime < 4000)
        {
          ...
          if (x_value > xUpperThresh)
            xUpperThresh = x_value;
          if (x_value < xLowerThresh)
            xLowerThresh = x_value;
          if (y_value > yUpperThresh)
            yUpperThresh = y_value;
          if (y_value < yLowerThresh)
            yLowerThresh = y_value;
  ...
  case 1:
    ...
    if (backAngle <= minBackAngle)
      backAngle = minBackAngle;
    break;
  case 2:
    ...
    if (footAngle >= maxFootAngle)
      footAngle = maxFootAngle;
    if (footAngle <= minFootAngle)
      footAngle = minFootAngle;
    break;
  default:
    break;
  }
